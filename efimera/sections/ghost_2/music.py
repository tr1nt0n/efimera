...
for voice_name in ["piano 3 voice", "piano 5 voice"]:
    ...
    abjad.attach(
        abjad.Dynamic("pppp"), abjad.select.leaves(score[voice_name], pitched=True)[0]
    )

    abjad.attach(
        abjad.StartHairpin("<"), abjad.select.leaves(score[voice_name], pitched=True)[0]
    )

    abjad.attach(abjad.Dynamic("p"), abjad.select.leaves(measures[8], pitched=True)[0])

    abjad.attach(
        abjad.StartHairpin(">o"), abjad.select.leaves(measures[12], pitched=True)[0]
    )
